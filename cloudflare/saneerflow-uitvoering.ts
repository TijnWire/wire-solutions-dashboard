// Wire Solutions — Saneren: clusters, ronden, antwoorden, poster en afronden (fase C t/m H)
// ─────────────────────────────────────────────────────────────────────────────
...
//   GET  /saneer/cluster?id=       alles van één cluster: adressen, ronde, antwoorden, beschikbaarheid
//   POST /saneer/ronde             nieuwe ronde starten voor een cluster
//   POST /saneer/respons           antwoord van één bewoner vastleggen (deur of telefoon)
//   POST /saneer/adres             naam, telefoon, belstatus of opmerking bijwerken
//   GET  /saneer/bellijst?pd=      adressen mét telefoonnummer, met belstatus
//   GET  /saneer/taken?pd=         posters en andere taken
//   POST /saneer/taak              taak afvinken of aanmaken
//   POST /saneer/afronden          dossier afronden of afboeken
//   GET  /saneer/export?pd=        alles van een dossier voor Excel en PDF

import type { SaneerContext } from "./saneerflow";
import { netPd } from "./saneerflow";
...
export async function saneerUitvoeringRoutes(
  ...
  const dossierVan = (pd: string) =>
    env.DB.prepare("select * from saneer_dossiers where pd_nummer = ? and verwijderd = 0")
  ...
  async function magBijCluster(clusterId: string) {
    ...
    const k = await env.DB.prepare("select * from saneer_clusters where id = ? and verwijderd = 0")
  ...
  if (pad === "/saneer/clusters/maak" && methode === "POST") {
    ...
    for (const pc of [...groepen.keys()].sort()) {
      ...
      if (!clusterId) {
        ...
        updates.push(env.DB.prepare("update saneer_clusters set naam = ?2, bijgewerkt_op = ?3 where id = ?1 and naam = ''")
          .bind(clusterId, naam, nuISO));
      }
      nieuweClusters.push({ id: clusterId, postcode: pc, naam, aantal: groep.length });
      for (const a of groep) {
        updates.push(env.DB.prepare("update saneer_adressen set cluster_id = ?2, volgorde = ?3, bijgewerkt_op = ?4 where id = ?1")
  ...
  if (pad === "/saneer/cluster" && methode === "POST") {
    ...
    const k = await env.DB.prepare("select * from saneer_clusters where id = ? and verwijderd = 0").bind(id).first<Record<string, unknown>>();
    ...
    await env.DB.prepare(`update saneer_clusters set ${zetten.join(", ")}, bijgewerkt_op = ?${zetten.length + 2} where id = ?1`)
  ...
  if (pad === "/saneer/cluster/splits" && methode === "POST") {
    ...
    const eerste = await env.DB.prepare("select postcode from saneer_adressen where id = ?").bind(ids[0]).first<{ postcode: string }>();
    ...
    for (let i = 0; i < ids.length; i += 30) {
      await env.DB.batch(ids.slice(i, i + 30).map((id) =>
        env.DB.prepare("update saneer_adressen set cluster_id = ?2, bijgewerkt_op = ?3 where id = ?1 and pd_nummer = ?4")
  ...
  if (pad === "/saneer/cluster" && methode === "GET") {
    ...
    const { results: responsen } = ronde
      ? await env.DB.prepare("select * from saneer_responsen where ronde_id = ?").bind(String(ronde.id)).all()
      : { results: [] as unknown[] };
  ...
  if (pad === "/saneer/ronde" && methode === "POST") {
    ...
    const vorige = await env.DB.prepare("select id, nummer from saneer_ronden where cluster_id = ? order by nummer desc limit 1")
      .bind(clusterId).first<{ id: string; nummer: number }>();
    const nummer = (vorige?.nummer ?? 0) + 1;
    const datum = String(body.voorgestelde_datum ?? "");

    if (vorige) {
      await env.DB.prepare("update saneer_ronden set actief = 0, afgesloten_op = ?2, uitkomst = case when uitkomst = '' then 'afgebroken' else uitkomst end where id = ?1")
  ...
  if (pad === "/saneer/respons" && methode === "POST") {
    ...
    const adres = await env.DB.prepare("select * from saneer_adressen where id = ? and verwijderd = 0").bind(adresId).first<Record<string, unknown>>();
    ...
    const ronde = meegestuurd
      ? await env.DB.prepare("select * from saneer_ronden where id = ?").bind(meegestuurd).first<Record<string, unknown>>()
      : await env.DB.prepare("select * from saneer_ronden where cluster_id = ? and actief = 1 order by nummer desc limit 1").bind(String(adres.cluster_id)).first<Record<string, unknown>>();
    ...
    await env.DB.prepare(
      "insert into saneer_responsen (id, ronde_id, adres_id, antwoord, via, opmerking, door, tijdstip, na_afsluiten) " +
    ...
    if (aanvul.length) {
      await env.DB.prepare(`update saneer_adressen set ${aanvul.join(", ")}, bijgewerkt_op = ?${aanvul.length + 2} where id = ?1`)
    ...
    for (let i = 0; i < datums.length; i += 30) {
      await env.DB.batch(datums.slice(i, i + 30).map(([d, kan]) =>
        env.DB.prepare(
          "insert into saneer_beschikbaarheid (id, adres_id, datum, kan, ronde_id, door, tijdstip) values (?1, ?2, ?3, ?4, ?5, ?6, ?7) " +
    ...
    const stand = await env.DB.prepare(
      ...
      "from saneer_adressen a left join saneer_responsen r on r.adres_id = a.id and r.ronde_id = ?2 " +
  ...
  if (pad === "/saneer/adres" && methode === "POST") {
    const id = String(body.id ?? "");
    const adres = await env.DB.prepare("select * from saneer_adressen where id = ? and verwijderd = 0").bind(id).first<Record<string, unknown>>();
    if (!adres) return json({ error: "Adres niet gevonden." }, 404);
    const toegang = await magBijCluster(String(adres.cluster_id ?? ""));
    if (toegang.fout) return toegang.fout;
  ...
  if (pad === "/saneer/bellijst" && methode === "GET") {
    ...
    const { results } = await env.DB.prepare(
      "select a.*, k.naam as cluster_naam, k.definitieve_datum from saneer_adressen a " +
      "left join saneer_clusters k on k.id = a.cluster_id " +
  ...
  if (pad === "/saneer/cluster/datum" && methode === "POST") {
    ...
    const ronde = await env.DB.prepare("select id, nummer from saneer_ronden where cluster_id = ? and actief = 1 order by nummer desc limit 1")
      .bind(clusterId).first<{ id: string; nummer: number }>();
    ...
    // DE POSTER — de taak ontstaat hier vanzelf, op het moment dat de afspraak rond is.
    // De regel: binnen twee weken ná het maken van de afspraak moet de aankondiging in het gebouw
    // hangen. Valt de uitvoering eerder dan die twee weken, dan telt de dag vóór de uitvoering — een
    // poster die pas ná de werkzaamheden hangt kondigt niets meer aan.
    ...
    const dagVoorUitvoering = new Date(`${datum}T12:00:00Z`);
    ...
    await env.DB.prepare(
      "insert into saneer_taken (id, pd_nummer, cluster_id, soort, deadline, bijgewerkt_op) values (?1, ?2, ?3, 'poster', ?4, ?5) " +
    ...
    const open = await env.DB.prepare("select count(*) as n from saneer_clusters where pd_nummer = ? and verwijderd = 0 and definitieve_datum = ''")
      .bind(pd).first<{ n: number }>();
    if (Number(open?.n ?? 0) === 0) {
      await env.DB.prepare("update saneer_dossiers set status = 'datum_akkoord', bijgewerkt_op = ?2 where pd_nummer = ?1 and status not in ('afgerond','afgeboekt')")
        .bind(pd, nuISO).run();
    }
  ...
  if (pad === "/saneer/taken" && methode === "GET") {
    ...
    const { results } = pd
      ? await env.DB.prepare(
          "select t.*, k.naam as cluster_naam, k.definitieve_datum, k.toegewezen_aan from saneer_taken t " +
          "left join saneer_clusters k on k.id = t.cluster_id where t.pd_nummer = ?1 order by t.deadline"
        ).bind(pd).all()
      // Zonder pd: alle openstaande taken, voor de herinnering bij het openen van de app.
      : await env.DB.prepare(
          "select t.*, k.naam as cluster_naam, k.definitieve_datum, k.toegewezen_aan, d.gebouw from saneer_taken t " +
          "left join saneer_clusters k on k.id = t.cluster_id " +
          "left join saneer_dossiers d on d.pd_nummer = t.pd_nummer " +
  ...
  if (pad === "/saneer/taak" && methode === "POST") {
    const id = String(body.id ?? "");
    const taak = await env.DB.prepare("select * from saneer_taken where id = ?").bind(id).first<Record<string, unknown>>();
    ...
    const open = await env.DB.prepare("select count(*) as n from saneer_taken where pd_nummer = ? and afgevinkt_op = ''").bind(pd).first<{ n: number }>();
    if (afvinken && Number(open?.n ?? 0) === 0) {
      await env.DB.prepare("update saneer_dossiers set status = 'poster_geplaatst', bijgewerkt_op = ?2 where pd_nummer = ?1 and status = 'datum_akkoord'")
        .bind(pd, nuISO).run();
    }
  ...
  if (pad === "/saneer/afronden" && methode === "POST") {
    ...
    const gaten = await env.DB.prepare(
      "select " +
      "(select count(*) from saneer_clusters where pd_nummer = ?1 and verwijderd = 0) as clusters, " +
