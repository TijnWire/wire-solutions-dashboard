-- Wire Solutions — Saneren: één uitvoeringsdatum per cluster
-- ─────────────────────────────────────────────────────────────────────────────
-- Draaien:  npx wrangler d1 execute wire-solutions --remote --file cloudflare/schema-saneerflow.sql
-- Terugdraaien: cloudflare/rollback/schema-saneerflow.rollback.sql
--
-- VOLLEDIG ADDITIEF: alleen nieuwe tabellen met een eigen voorvoegsel (saneer_). Geen bestaande
...
-- Een nieuwe ronde is een rij erbij in saneer_ronden. Naam, telefoonnummer en opmerkingen staan in
-- saneer_adressen en worden daarbij niet aangeraakt — er is geen code die dat kán.

-- ─────────────────────────────────────────────────────────────────────────────
-- 1) DOSSIER — alles hangt onder het PD-nummer, zodat het aan het eind in één keer afgeboekt kan worden.
create table if not exists saneer_dossiers (
  ...
  poster_weken_voor integer not null default 2,   -- poster ophangen binnen zoveel weken na de afspraak
...
create table if not exists saneer_clusters (
  id                text primary key,
...
create index if not exists saneer_clusters_dossier_idx on saneer_clusters (pd_nummer, postcode);

-- ─────────────────────────────────────────────────────────────────────────────
-- 3) ADRES — de vaste gegevens. Deze tabel overleeft elke nieuwe ronde ongeschonden.
create table if not exists saneer_adressen (
  id             text primary key,
  pd_nummer      text not null,
...
create index if not exists saneer_adressen_dossier_idx on saneer_adressen (pd_nummer, volgorde);
create index if not exists saneer_adressen_cluster_idx on saneer_adressen (cluster_id, volgorde);

-- ─────────────────────────────────────────────────────────────────────────────
-- 4) RONDE — één poging om een datum rond te krijgen voor één cluster.
create table if not exists saneer_ronden (
  id                 text primary key,
...
create index if not exists saneer_ronden_cluster_idx on saneer_ronden (cluster_id, nummer);

-- ─────────────────────────────────────────────────────────────────────────────
-- 5) RESPONS — het antwoord van één bewoner in één ronde. Nieuwe ronde = nieuwe responsen;
--    de oude blijven staan als historie.
create table if not exists saneer_responsen (
  id           text primary key,
...
create unique index if not exists saneer_responsen_uniek on saneer_responsen (ronde_id, adres_id);
create index if not exists saneer_responsen_adres_idx on saneer_responsen (adres_id);
...
create table if not exists saneer_beschikbaarheid (
  id           text primary key,
  adres_id     text not null,
...
create unique index if not exists saneer_beschikbaarheid_uniek on saneer_beschikbaarheid (adres_id, datum);

-- ─────────────────────────────────────────────────────────────────────────────
-- 7) TAAK — nu alleen de poster, later eventueel meer.
create table if not exists saneer_taken (
  id            text primary key,
  pd_nummer     text not null,
  cluster_id    text not null default '',
...
create index if not exists saneer_taken_dossier_idx on saneer_taken (pd_nummer, afgevinkt_op);
...
create table if not exists saneer_log (
...
create index if not exists saneer_log_dossier_idx on saneer_log (pd_nummer, tijdstip);

-- ── Controle ──
--   select pd_nummer, status, regio from saneer_dossiers where verwijderd = 0;
--   select gebeurtenis, count(*) from saneer_log group by 1;


-- Wire Solutions — Saneren: import onthouden en afgekeurde regels bewaren
-- Draaien:  npx wrangler d1 execute wire-solutions --remote --file cloudflare/schema-saneerflow.sql
...
create table if not exists saneer_mappings (
...
create table if not exists saneer_afgekeurd (
  id          text primary key,
  pd_nummer   text not null,
...
create index if not exists saneer_afgekeurd_dossier_idx on saneer_afgekeurd (pd_nummer, opgelost);
