...
async function main() {
  ...
  try {
    ...
    // ── 7. Bewonersakkoord: import vult aan, overschrijft nooit ──
    console.log("\n7. Bewonersakkoord — import");
    let PD2 = "";
    let cluster1 = "";
    {
      PD2 = `PD${String(stempel).slice(-6)}7`;
      await post("/akkoord/dossier", baas, {
      ...
      const eerste = await post("/akkoord/adressen", baas, {
      ...
      const na = await get(`/akkoord/adressen?pd=${PD2}`, baas);
      ...
      const onthouden = await get("/akkoord/mapping?opdrachtgever=Stedin", baas);
      check(onthouden.data.mapping?.huisnummer === 1, "de kolomindeling is onthouden voor de volgende keer");
    }

    // ── 8. Clusteren en verdelen ──
    console.log("\n8. Bewonersakkoord — clusters");
    {
      const r = await post("/akkoord/clusters/maak", baas, { pd_nummer: PD2 });
      ...
      const opnieuw = await post("/akkoord/clusters/maak", baas, { pd_nummer: PD2 });
      check(opnieuw.data.clusters?.length === 2, "nog eens clusteren maakt geen dubbele clusters");

      const geenRecht = await post("/akkoord/cluster", mont, { id: cluster1, toegewezen_aan: "u2" });
      check(geenRecht.status === 403, "een monteur mag zichzelf geen cluster toewijzen");
      await post("/akkoord/cluster", baas, { id: cluster1, toegewezen_aan: "u2" });
      const eigen = await get(`/akkoord/adressen?pd=${PD2}`, mont);
    ...
    console.log("\n9. Bewonersakkoord — één datum voor iedereen");
    {
      const r = await post("/akkoord/ronde", mont, { cluster_id: cluster1, voorgestelde_datum: "2026-10-07" });
      check(r.status === 200 && r.data.nummer === 1, "ronde 1 gestart", r.data.error ?? "");

      const antwoord = (n, a, extra = {}) => post("/akkoord/respons", mont, {
        adres_id: `${PD2}-a${n}`, antwoord: a, via: "deur", ...extra,
      });
      await antwoord(1, "akkoord", { bewoner: "Fam. Jansen", kan_wel: ["2026-10-07", "2026-10-08"] });
      const half = await post("/akkoord/cluster/datum", mont, { cluster_id: cluster1, datum: "2026-10-07" });
      ...
      const tegen = await post("/akkoord/cluster/datum", mont, { cluster_id: cluster1, datum: "2026-10-07" });
      check(tegen.status === 409, "één bewoner die niet kan blokkeert de datum");

      // Nieuwe ronde met een datum die iedereen wél kan.
      const r2 = await post("/akkoord/ronde", mont, { cluster_id: cluster1, voorgestelde_datum: "2026-10-08" });
      check(r2.data.nummer === 2, "ronde 2 gestart");
      const bewaard = await get(`/akkoord/cluster?id=${cluster1}`, mont);
      ...
      const vast = await post("/akkoord/cluster/datum", mont, { cluster_id: cluster1, datum: "2026-10-08" });
      check(vast.status === 200, "met iedereen akkoord komt de datum erin", vast.data.error ?? "");
      check(vast.data.poster_deadline === "2026-09-24", "de postertaak krijgt een deadline twee weken ervoor", vast.data.poster_deadline);
    }

    // ── 10. Poster en afronden ──
    console.log("\n10. Bewonersakkoord — poster en afronden");
    {
      const teVroeg = await post("/akkoord/afronden", baas, { pd_nummer: PD2 });
      ...
      const taken = await get(`/akkoord/taken?pd=${PD2}`, baas);
      check(taken.data.taken?.length === 1, "er staat een postertaak klaar");
      await post("/akkoord/taak", baas, { id: taken.data.taken[0].id, notitie: "Opgehangen bij de brievenbussen" });

      const nog = await post("/akkoord/afronden", baas, { pd_nummer: PD2 });
      check(nog.status === 409 && (nog.data.belet ?? []).some((b) => /datum/i.test(b)), "het tweede cluster zonder datum houdt afronden tegen");

      const ex = await get(`/akkoord/export?pd=${PD2}`, baas);
