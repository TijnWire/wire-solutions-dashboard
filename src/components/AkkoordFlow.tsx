...
import { AkkoordImport } from "./AkkoordImport";
import { AkkoordVerdelen, AkkoordClusterWerk } from "./AkkoordCluster";
import { haalDossier, STATUS_INFO, type Dossier, type DossierDetail } from "../lib/akkoord";
import {
  haalAkkoordAdressen, haalBellijst, haalTaken, vinkTaak, wijzigAkkoordAdres, rondDossierAf,
  haalExport, BELSTATUS_INFO, type AkkoordAdres, type Taak,
} from "../lib/akkoordWerk";
import { exporteerAkkoordExcel, exporteerAkkoordPdf } from "../lib/akkoordExport";

// Bewonersakkoord — één dossier, stap voor stap.
// ─────────────────────────────────────────────────────────────────────────────
...
export function AkkoordFlow({ pd, onTerug }: { pd: string; onTerug: () => void }) {
  const { users, currentUser } = useApp();
  const [detail, setDetail] = useState<DossierDetail | null>(null);
  const [adressen, setAdressen] = useState<AkkoordAdres[]>([]);
  ...
  const laad = useCallback(async () => {
    const [d, a, t] = await Promise.all([haalDossier(pd), haalAkkoordAdressen(pd), haalTaken(pd)]);
  ...
  return (
    <div className="space-y-4">
      ...
      {actief === "inlezen" && <AkkoordImport dossier={dossier} aantalNu={adressen.length} onKlaar={() => void laad()} />}
      ...
      {actief === "verdelen" && (
        adressen.length === 0
          ...
          : <AkkoordVerdelen dossier={dossier} adressen={adressen} clusters={clusters} veldwerkers={veldwerkers} naamVan={naamVan} onWijzig={() => void laad()} />
      )}
...
function Bellijst({ pd, onWijzig }: { pd: string; onWijzig: () => void }) {
  const [lijst, setLijst] = useState<AkkoordAdres[] | null>(null);
  const laad = useCallback(async () => setLijst(await haalBellijst(pd)), [pd]);
  useEffect(() => { void laad(); }, [laad]);

  async function zet(a: AkkoordAdres, belstatus: string) {
    await wijzigAkkoordAdres(a.id, { belstatus, belpogingen: (a.belpogingen ?? 0) + (belstatus === "geen_gehoor" ? 1 : 0) });
    void laad(); onWijzig();
  }
...
function Afronden({ dossier, clusters, taken, onWijzig }: {
  ...
  async function exporteer(soort: "xlsx" | "pdf") {
    ...
    if (soort === "xlsx") await exporteerAkkoordExcel(data); else exporteerAkkoordPdf(data);
    setBezig("");
  }
