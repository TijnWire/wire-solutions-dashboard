// Bewonersakkoord — exporteren naar Excel en PDF.
// ─────────────────────────────────────────────────────────────────────────────
...
import { ANTWOORD_INFO, BELSTATUS_INFO, type ExportData, type AkkoordAdres } from "./akkoordWerk";
...
const bestandsnaam = (d: ExportData, ext: string) =>
  `${d.dossier.pd_nummer} bewonersakkoord ${new Date().toISOString().slice(0, 10)}.${ext}`;

// ── Excel ── vier bladen: adressen, groepen, alle antwoorden per ronde, en het wijzigingslog.
export async function exporteerAkkoordExcel(d: ExportData): Promise<void> {
...
export function exporteerAkkoordPdf(d: ExportData): void {
  ...
  const koptekst = (vervolg = false) => {
    ...
    doc.text(`Bewonersakkoord ${d.dossier.pd_nummer}${vervolg ? " (vervolg)" : ""}`, M, 28);
